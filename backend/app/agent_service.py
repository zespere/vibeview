...
class AgentService:
    ...
    def run_workspace_prompt(
        ...
        canvas_snapshot_text = json.dumps(canvas_snapshot, indent=2, ensure_ascii=True) if canvas_snapshot else None
        workspace_prompt_lines = [
            ...
            "- If the user asks to show, map, expose, represent, or refine codebase structure on the canvas, you must update the current active canvas in canvases.json directly instead of implementing a new UI feature.",
            "- For canvas-oriented requests, finishing without modifying the active canvas is a failure unless it is genuinely impossible. If impossible, explain exactly why.",
            ...
            "- Always end with a brief natural-language response summarizing what you changed or found.",
        ]
        if canvas_snapshot_text:
            workspace_prompt_lines.extend([
                "",
                "Current active canvas JSON:",
